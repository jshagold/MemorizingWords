...
import com.example.memorizingwords.AppiumViewerArticleListTest.Companion
...
// 4) 수행
    ...
    @Test
    fun test_014_Click_AI_Translate_Button() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )


        val aiTranslateBtn = driver.findElement(AppiumBy.id("com.plantym.mediaservice.moazine:id/menu_ai_translation") )
        aiTranslateBtn.click()
    }

    @Test
    fun test_015_Select_Language_From_List() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )

        val n = 1   // 0-based index you want
        val locator = AppiumBy.androidUIAutomator(
            // 한 줄 문자열이어야 파서가 오류 없이 읽습니다
            "new UiScrollable(new UiSelector().resourceId(\"com.plantym.mediaservice.moazine:id/rv_language_list\"))" +
    ...
    @Test
    fun test_016_Click_Translate_Button() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )


        val translateBtn = driver.findElement(AppiumBy.id("com.plantym.mediaservice.moazine:id/button_do_translate") )
        translateBtn.click()
    }


    @Test
    fun test_017_Click_Next_Article_Button() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )


        val nextArticleBtn = driver.findElement(AppiumBy.id("com.plantym.mediaservice.moazine:id/toc_direction_right_btn") )
        nextArticleBtn.click()
    }

    @Test
    fun test_018_Click_Next_Article_Button() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )
    ...
    @Test
    fun test_019_Click_Prev_Article_Button() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )


        val prevArticleBtn = driver.findElement(AppiumBy.id("com.plantym.mediaservice.moazine:id/toc_direction_left_btn") )
        prevArticleBtn.click()
    }

    @Test
    fun test_020_Click_Prev_Article_Button() {
        Thread.sleep(testLoadingTime)
        println("Native App: ${driver.pageSource}", )
